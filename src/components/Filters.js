...
const Filters = () => {
  const {filters:{text,category,company,min_price,max_price,price,shipping},
        ...
        <div className="content">
          <form onSubmit={(e)=> e.preventDefault()}>
            ...
            <div className="form-control">
              ...
              <div>{
                categories.map((c,i)=> {
                    return <button key={i} onClick={updateFilters} type="button"name="category"className={`${category === c.toLowerCase() ?
                    'active' : null}`}>{c}</button>
                })
                }</div>
            </div>
            {/* categories */}
